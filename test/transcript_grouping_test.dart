...
CommandExecutionItem _cmd(String id) => CommandExecutionItem(id, command: 'ls');
...
ContextCompactionItem _compact(String id) => ContextCompactionItem(id);

void main() {
  group('buildTranscriptEntries', () {
    test(
      'folds a turn\'s steps into a group, keeping the final agent reply out',
      () {
    ...
    test(
      'interstitial agent notes fold into the group, only the last is out',
      () {
    ...
    test('trailing group is live when the turn is running', () {
      final items = <ThreadItem>[_cmd('c1'), _edit('e1')];
      final group =
          buildTranscriptEntries(items, turnRunning: true).single
              as StepGroupEntry;
      expect(group.live, isTrue);
    });

    test(
      'while running, the whole trailing segment stays in the live group',
      () {
    ...
    test(
      'once the turn finishes, the final agent reply peels out standalone',
      () {
    ...
    test(
      'an already-complete earlier segment peels even while a later turn runs',
      () {
    ...
    test('compaction does not fold the preceding agent reply into steps', () {
      final items = <ThreadItem>[
        _user('u1'),
        _cmd('c1'),
        _agent('a1'),
      ...
      expect((entries[1] as StepGroupEntry).items.single.id, 'c1');
      expect((entries[2] as ItemEntry).item.id, 'a1');
      expect((entries[3] as ItemEntry).item.id, 'compact1');
      expect((entries[4] as ItemEntry).item.id, 'u2');
    });
